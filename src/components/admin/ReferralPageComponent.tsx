...
export default function ReferralPageComponent() {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      setError(null);
      try {
        // Only fetch the main referrer's info on the first page load
        if (currentPage === 1 && !referrer) {
          const referrerData = await getUserById(userId);
          setReferrer(referrerData);
        }

        // Fetch the referrals for the current page
        const referralsData = await getReferralsByUserId(
          userId,
          currentPage,
          pageSize
        );
